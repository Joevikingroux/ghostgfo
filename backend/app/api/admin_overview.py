...
def admin_overview(
        ...
        clients.append(
            {
                ...
                "last_report_generated": report.generated_at.isoformat()
                ...
                "agent_last_heartbeat": agent.last_heartbeat_at.isoformat()
                if agent and agent.last_heartbeat_at
                else None,
                "agent_last_sync": agent.last_sync_at.isoformat()
                if agent and agent.last_sync_at
                else None,
    ...
    health_dist = dict(
        Counter(c["health_rating"] for c in clients if c["health_rating"])
    )
